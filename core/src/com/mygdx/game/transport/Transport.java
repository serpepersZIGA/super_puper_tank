...
public abstract class Transport{
    ...
    damage,penetration,damage_fragment,penetration_fragment,t,t_damage,armor,hill,fire_x,fire_y, width_tower, height_tower,aim_x,aim_y;
    ...
    protected File sound_fire;
    protected double slowing = 0.05,speed_minimum;
    ...
    private double g,g_right, g_left;
    public boolean host;

    private int i;
    protected int distance_target = 200;
    protected int distance_target_2 = 230;
    public double difference_x,difference_y,green_len;
    public double rotation_fire;
    ...
    private void peaceful_behavior(){
        ...
        if(this.time_relocation<0){
            if(rand.rand(2)== 1) {
                this.x_relocation = this.x + (200.0+rand.rand(300.0));
            }
            else{this.x_relocation = this.x + (-500+rand.rand(300));}
            if(rand.rand(2)== 1) {
                this.y_relocation = this.y + (200.0+rand.rand(300));
            }
            else{this.y_relocation = this.y + -500+rand.rand(300);}
            for (i = 0; i< Main.BuildingList.size(); i++){
                if(Main.BuildingList.get(i).x- Main.BuildingList.get(i).width_2>this.x_relocation & Main.BuildingList.get(i).x+ Main.BuildingList.get(i).width<this.x_relocation){
                    if(Main.BuildingList.get(i).y- Main.BuildingList.get(i).height_2>this.y_relocation & Main.BuildingList.get(i).y+ Main.BuildingList.get(i).height<this.y_relocation) {
                        if(rand.rand(2)== 1) {
                            this.x_relocation = this.x + (200.0+rand.rand(300));
                        }
                        else{this.x_relocation = this.x + (-500+rand.rand(300));}
                        if(rand.rand(2)== 1) {
                            this.y_relocation = this.y + (200.0+rand.rand(300));
    ...
    protected void tower_player() {
        this.rotation_tower = Method.tower_player(this.x_tower_rend+this.tower_width_2,this.y_tower_rend+this.tower_height_2, this.rotation_tower, this.speed_tower);
    }
    protected void tower_player_2() {
        this.rotation_tower = Method.tower_player(RC.width_2, RC.height_2, this.rotation_tower, this.speed_tower);
    }
...
//    }
    ...
    protected void motor_bot_bypass(int i) {
        if(PlayerList.size() != 0) {
            ...
            double[]list = less_hp_bot(spisok,enemy_spisok,spisok,i);
            g=list[0];
            double e =list[1];
            int i2 = (int) list[2];
    ...
    protected void indicator_hp(){
        green_len = ((double)this.hp/this.max_hp)* option.size_x_indicator;
    ...
    protected void fire_player_bull_tank(){
        ...
        if(this.left_mouse && this.reload <= 0){
            for (Transport transport : this.enemy_spisok) {
                if (sqrt(pow(this.x - transport.x, 2) + pow(this.y - transport.y, 2)) < transport.range_see_2) {
    ...
    double []xy = Method.tower_xy(this.x,this.y,this.tower_x_const,this.tower_y_const,this.difference,-this.rotation_corpus);
        this.tower_x = xy[0];this.tower_y = xy[1];}
    protected void tower_xy_2(){
        double []xy = Method.tower_xy_2(this.x,this.y,this.tower_x_const,this.tower_y_const,this.difference,this.difference_2,-this.rotation_corpus);
        this.tower_x = xy[0];this.tower_y = xy[1];}
        ...
        if(obj_building.size()!= 0&& this.enemy_spisok.size()!=0) {
            if (ai_sost == 0) {
                if (null != findIntersection(this.tower_x, this.tower_y, obj_tr.get(i2).tower_x, obj_tr.get(i2).tower_y)) {
                    double[] xy = Method.tower_xy_2(this.x, this.y, this.ai_x_const, this.ai_y_const, 0, 0, -this.rotation_corpus);
            ...
            if(path.size() > 0) {
                double []xy = Method.tower_xy_2(this.x,this.y,this.ai_x_const,this.ai_y_const,0,0,-this.rotation_corpus);
                this.g = sqrt(pow((xy[0] - BlockList2D.get(path.get(0)[1]).get(path.get(0)[0]).x_center), 2) + pow(xy[1] - BlockList2D.get(path.get(0)[1]).get(path.get(0)[0]).y_center, 2));
                ...
                double rad = sqrt(pow((spisok.get(i3).x - obj_tr.get(i2).x), 2) + pow(spisok.get(i3).y - obj_tr.get(i2).y, 2));
    ...
    protected double[] findIntersection(double x0, double y0, double dx, double dy, double x1, double y1, double x2, double y2) {
    ...
    private double[] findIntersection(double x0, double y0, double dx, double dy) {
        double xy_r = atan2(dy-y0, dx-x0);
        double speed_x = 10 * cos(xy_r);
        double speed_y = 10 * sin(xy_r);
        int BlockX;
    ...
    private double[] less_hp_bot(ArrayList<Transport>bot, ArrayList<Transport>obj_player, ArrayList<Transport>obj_support, int i){
        int i2;
        if (this.hp > this.max_hp / 3 && this.medic_help == 0) {
            ...
            if(ind != NULL){
                ...
                g -= 90;
                return new double[]{g,2,ind};
            }
            i2 = Method.detection_near_transport_i(bot,i,obj_player);
            g = atan2(this.y - obj_player.get(i2).y, this.x - obj_player.get(i2).x) / 3.1415926535 * 180;
            g -= 90;
            return new double[]{g,1,ind};
        }
    ...
    private void physic_collision(ArrayList<Transport>obj_2, int i){
        ...
        if(sqrt(pow(x_1_2 - x_2_2, 2) + pow(y_1_2 - y_2_2, 2))<(obj_2.get(i).corpus_width_2+corpus_width_2)*1.4){
            xy = Method.tower_xy_2(x_2,y_2,0,0,-obj_2.get(i).corpus_height_2/1.5,obj_2.get(i).corpus_width_2*1.2,-obj_2.get(i).rotation_corpus);
            double x_2_2_1 = xy[0];
            double y_2_2_1 = xy[1];
            xy = Method.tower_xy_2(x_2,y_2,0,0,-obj_2.get(i).corpus_height_2/1.5,-obj_2.get(i).corpus_width_2*1.2,-obj_2.get(i).rotation_corpus);
            double x_2_2_2 = xy[0];
            double y_2_2_2 = xy[1];
            xy = Method.tower_xy_2(x,y,0,0,-corpus_height_2/1.5,corpus_width_2*1.2,-rotation_corpus);
            double x_1_2_1 = xy[0];
            double y_1_2_1 = xy[1];
            xy = Method.tower_xy_2(x,y,0,0,-corpus_height_2/1.5,-corpus_width_2*1.2,-rotation_corpus);
            double x_1_2_2 = xy[0];
    ...
    public void tower_action_client(int i,double x,double y,double rotation,boolean sost_fire_bot,boolean sost_fire_bot_2){
